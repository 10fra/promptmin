...
test("preserve tag survives minimization (ddmin)", async () => {
  const pkgDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
  const repoRoot = path.resolve(pkgDir, "..", "..");
  const cliPath = path.join(pkgDir, "dist", "cli.js");

  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "promptmin-preserve-"));
  ...
  await fs.writeFile(
    promptPath,
    ["# Test", "", "<!-- promptmin:keep -->", "KEEP_ME", "", "BAD_TOKEN", ""].join("\n"),
    "utf8",
  );

  const python = [
    "import os; p=os.environ.get('PROMPT_TEXT','');",
    "print('BAD' if 'BAD_TOKEN' in p else 'OK')",
  ].join(" ");
  ...
  assert.match(minimized, /promptmin:keep/);
